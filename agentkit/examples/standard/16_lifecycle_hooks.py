"""
16_lifecycle_hooks.py — 生命周期 Hooks 与 Callbacks
...
async def before_agent(ctx):
    ...
    # 如果这里返回了内容，会直接中断 Agent 执行，将返回内容作为结果
    # return "直接拦截"
...
async def main():
    ...
    print("\n--- 异常捕获测试 ---")
    # 触发工具异常，会被 try-except 包装并由 LLM 看到，不触发 on_error，
    # 但如果是大模型调用失败，或者 Hook 抛出异常，会被处理。
    # 这里我们在 after_model 增加一个故意报错的 Hook
