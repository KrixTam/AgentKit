...
BASE_MODEL = resolve_model("gpt-4o-mini")
OVERRIDE_MODEL = f"{BASE_MODEL}-override"
...
def main() -> None:
    ...
    try:
        LockedStandardAgent(name="locked-agent", model=OVERRIDE_MODEL)
        print("❌ 预期失败，但实例化成功了")
    except ValueError as e:
        print(f"✅ 拒绝覆盖成功: {e}")

    print("\n=== 2) 开启能力：实例化覆盖应成功 ===")
    cosplay_agent = CosplayStandardAgent(name="cosplay-agent", model=OVERRIDE_MODEL)
