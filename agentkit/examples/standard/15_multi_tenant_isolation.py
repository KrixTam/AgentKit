"""
15_multi_tenant_isolation.py — 多租户与资源隔离测试
...
# === 1. 定义测试用 Skill ===
# 这个 Skill 会在 on_load 时初始化一个只属于当前 Session 的"计数器"
...
def increment_counter(ctx) -> str:
    """增加当前 Session 的计数器"""
    # 由于这是一个普通工具，我们要通过某种方式拿到 skill 的 context
    # 注意：在真实 Skill 中，通常是在额外工具列表中闭包传入 skill 实例，这里我们简化，直接从 ctx.state 取
...
async def main():
    # 使用本地字典模拟 Mem0，避免依赖外部服务
    # 真实场景使用 Mem0Provider()
    class MockMemory(Mem0Provider):
        def __init__(self):
            self.db = {} # {user_id: [messages]}
