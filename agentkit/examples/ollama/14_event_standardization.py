...
async def main():
    ...
    async for event in Runner.run_streamed(agent, input="开始计算"):
        ...
        if event.type == EventType.LLM_RESPONSE:
            ...
            print(f"[{event.type}] 准备调用工具: {event.data.get('tool')}")
            ...
            try:
                ...
                print(f"[{event.type}] 工具校验成功 -> 工具名: {validated_data.tool}, 结果: {validated_data.result}")
            except ValueError as e:
                print(f"[{event.type}] 工具结果校验失败: {e}")
                
        elif event.type == EventType.FINAL_OUTPUT:
            print(f"\n[{event.type}] 最终结果: {event.data}")
            
        elif event.type == EventType.ERROR:
            print(f"[{event.type}] 发生错误: {event.data}")
